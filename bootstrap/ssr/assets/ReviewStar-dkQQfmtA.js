import { jsx, Fragment, jsxs } from "react/jsx-runtime";
import "react";
function Star({ fill, width, height }) {
  return /* @__PURE__ */ jsx(Fragment, { children: /* @__PURE__ */ jsxs(
    "svg",
    {
      fill: fill || "#000000",
      width: width || "800px",
      height: height || "800px",
      viewBox: "0 0 32 32",
      version: "1.1",
      stroke: fill,
      xmlns: "http://www.w3.org/2000/svg",
      children: [
        /* @__PURE__ */ jsx("title", { children: "star" }),
        /* @__PURE__ */ jsx("path", { d: "M30.383 12.699c-0.1-0.303-0.381-0.519-0.713-0.519-0 0-0 0-0 0h-9.898l-3.059-9.412c-0.124-0.276-0.396-0.464-0.713-0.464s-0.589 0.189-0.711 0.459l-0.002 0.005-3.059 9.412h-9.897c-0.414 0-0.749 0.336-0.749 0.75 0 0.248 0.121 0.469 0.307 0.605l0.002 0.001 8.007 5.818-3.059 9.412c-0.023 0.070-0.037 0.15-0.037 0.233 0 0.414 0.336 0.75 0.75 0.75 0.165 0 0.318-0.054 0.442-0.144l-0.002 0.001 8.008-5.818 8.006 5.818c0.122 0.090 0.275 0.144 0.441 0.144 0.414 0 0.75-0.336 0.75-0.75 0-0.083-0.014-0.164-0.039-0.239l0.002 0.005-3.059-9.412 8.010-5.818c0.188-0.138 0.308-0.357 0.308-0.605 0-0.083-0.014-0.163-0.038-0.238l0.002 0.005zM20.779 18.461c-0.188 0.138-0.309 0.358-0.309 0.607 0 0.083 0.014 0.163 0.039 0.238l-0.002-0.005 2.514 7.736-6.581-4.783c-0.122-0.089-0.275-0.143-0.44-0.143s-0.318 0.053-0.443 0.144l0.002-0.002-6.581 4.783 2.514-7.736c0.024-0.070 0.037-0.15 0.037-0.233 0-0.249-0.121-0.469-0.307-0.605l-0.002-0.001-6.58-4.78h8.134c0 0 0.001 0 0.001 0 0.331 0 0.612-0.215 0.71-0.513l0.002-0.005 2.514-7.735 2.514 7.735c0.1 0.303 0.381 0.519 0.713 0.519 0 0 0 0 0 0h8.135z" })
      ]
    }
  ) });
}
function StarFill({ fill, width, height }) {
  return /* @__PURE__ */ jsxs(
    "svg",
    {
      fill: fill || "#000000",
      width: width || "800px",
      height: height || "800px",
      viewBox: "0 0 32 32",
      version: "1.1",
      xmlns: "http://www.w3.org/2000/svg",
      children: [
        /* @__PURE__ */ jsx("title", { children: "star" }),
        /* @__PURE__ */ jsx("path", { d: "M3.488 13.184l6.272 6.112-1.472 8.608 7.712-4.064 7.712 4.064-1.472-8.608 6.272-6.112-8.64-1.248-3.872-7.808-3.872 7.808z" })
      ]
    }
  );
}
function StarHalf({ fill, width, height }) {
  return /* @__PURE__ */ jsx(Fragment, { children: /* @__PURE__ */ jsxs(
    "svg",
    {
      fill: fill || "#000000",
      width: width || "800px",
      height: height || "800px",
      viewBox: "0 0 32 32",
      version: "1.1",
      stroke: fill,
      xmlns: "http://www.w3.org/2000/svg",
      children: [
        /* @__PURE__ */ jsx("title", { children: "star-half" }),
        /* @__PURE__ */ jsx("path", { fill, d: "M16.75 3l-0.007-0.033c-0.005-0.056-0.016-0.107-0.032-0.156l0.001 0.005c-0.010-0.050-0.023-0.094-0.039-0.137l0.002 0.005c-0.024-0.044-0.050-0.082-0.079-0.117l0.001 0.001c-0.028-0.043-0.057-0.080-0.090-0.115l0 0c-0.031-0.026-0.066-0.049-0.103-0.070l-0.004-0.002c-0.041-0.030-0.088-0.057-0.138-0.079l-0.005-0.002-0.027-0.015c-0.033-0.007-0.071-0.011-0.111-0.013l-0.001-0c-0.034-0.010-0.075-0.019-0.116-0.024l-0.003-0-0.032 0.007c-0.055 0.005-0.106 0.016-0.155 0.032l0.005-0.001c-0.051 0.010-0.096 0.023-0.138 0.040l0.005-0.002c-0.043 0.024-0.080 0.049-0.114 0.077l0.001-0.001c-0.044 0.028-0.082 0.058-0.117 0.092l0-0c-0.025 0.031-0.048 0.065-0.069 0.101l-0.002 0.004c-0.031 0.042-0.058 0.089-0.079 0.139l-0.002 0.005-0.015 0.027-3.059 9.412h-9.897c-0.414 0-0.749 0.336-0.749 0.75 0 0.248 0.121 0.469 0.307 0.605l0.002 0.001 8.007 5.818-3.059 9.412c-0.023 0.069-0.037 0.149-0.037 0.232 0 0.414 0.336 0.75 0.75 0.75 0.165 0 0.318-0.053 0.442-0.144l-0.002 0.001 8.448-6.139c0.017-0.012 0.025-0.030 0.040-0.044 0.070-0.066 0.13-0.141 0.178-0.223l0.003-0.005c0.037-0.082 0.065-0.176 0.077-0.275l0-0.005c0.001-0.019 0.011-0.036 0.011-0.056zM11.492 19.301c0.024-0.070 0.037-0.15 0.037-0.233 0-0.249-0.121-0.469-0.307-0.605l-0.002-0.001-6.58-4.78h8.134c0 0 0.001 0 0.001 0 0.331 0 0.612-0.215 0.71-0.513l0.002-0.005 1.764-5.428v14.745l-6.271 4.558z" })
      ]
    }
  ) });
}
function ReviewStar({ rating }) {
  const averageRating = rating || 1;
  const fullStars = Math.floor(averageRating);
  const hasHalfStar = averageRating % 1 !== 0;
  const renderStars = () => {
    const stars = [];
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        /* @__PURE__ */ jsx(
          StarFill,
          {
            fill: "rgb(253, 224, 71)",
            width: "1.3rem",
            height: "1.3rem"
          },
          i
        )
      );
    }
    if (hasHalfStar) {
      stars.push(
        /* @__PURE__ */ jsx(
          StarHalf,
          {
            fill: "rgb(253, 224, 71)",
            width: "1rem",
            height: "1rem"
          },
          "half"
        )
      );
    }
    const remainingStars = 5 - Math.ceil(averageRating);
    for (let i = 0; i < remainingStars; i++) {
      stars.push(
        /* @__PURE__ */ jsx(
          Star,
          {
            fill: "rgb(253, 224, 71)",
            width: "1rem",
            height: "1rem"
          },
          `empty-${i}`
        )
      );
    }
    return stars;
  };
  return /* @__PURE__ */ jsx("span", { className: "flex justify-start items-center", children: renderStars() });
}
export {
  ReviewStar as R
};
